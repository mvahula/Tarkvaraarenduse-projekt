...
# ---------------------------------------------------------
# PAC-MAN STIILIS MÄNG
# ---------------------------------------------------------
# See mäng on tehtud Pygame'iga ja on Pac-Manist inspireeritud.
# Mängija liigub labürindis, sööb punkte ja väldib kummitusi.
# Kui mängija sööb suure kollase boonuse, saab ta ajutiselt kummitusi süüa.
#
# Täiendused/kombineerimised:
# 1. Punktide kogumine
# 2. Kummitused
# 3. Boonustoit ehk power pellet
# 4. Elude süsteem
# 5. Tasemete süsteem
# 6. Paus
# 7. Võidu- ja kaotusekraan
# ---------------------------------------------------------


# värvid
# Kõik värvid on RGB kujul.
# Esimene number on punane, teine roheline ja kolmas sinine.
black = [0, 0, 0]
white = [255, 255, 255]
yellow = [255, 220, 0]
blue = [0, 0, 180]
...
# o = suur boonuspunkt
# P = mängija alguskoht
# G = kummituse alguskoht
# tühi koht = vaba tee
#
# Mäng on ruudustiku põhine.
# Iga märk muutub mängus üheks ruuduks.
RAW_MAP = [
    "####################",
    "#P......##......G..#",
    "#.####..##..####...#",
...
# ekraani seaded
# TILE määrab ühe kaardiruudu suuruse pikslites.
# Kuna kaart on 20 ruutu lai ja 16 ruutu kõrge,
# siis mänguväli tuleb 20 * 30 = 600 pikslit lai
# ja 16 * 30 = 480 pikslit kõrge.
TILE = 30
HUD_HEIGHT = 60
...
# fondid
# Väiksemat fonti kasutatakse skoori, elude ja taseme kuvamiseks.
# Suuremat fonti kasutatakse menüüdes ja mängu lõpus.
font = pygame.font.SysFont(None, 32)
bigFont = pygame.font.SysFont(None, 58)


# mängu seaded
# FPS määrab, kui mitu korda sekundis mängu uuendatakse.
# MOVE_DELAY määrab, kui kiiresti mängija liigub ruudustiku kaupa.
FPS = 60
MOVE_DELAY = 8
...
def drawText(text, color, x, y, fontType):
    # See funktsioon teeb teksti kuvamise lihtsamaks.
    # text on tekst, mida näidatakse.
    # color on teksti värv.
    # x ja y määravad teksti asukoha ekraanil.
    # fontType määrab, millise suurusega fonti kasutatakse.
    img = fontType.render(text, True, color)
    screen.blit(img, [x, y])


def gridToPixel(gridX, gridY):
    # See funktsioon muudab ruudustiku koordinaadid piksliteks.
    # Näiteks kui gridX on 2, siis pikslites on x = 2 * TILE.
    # HUD_HEIGHT lisatakse y-koordinaadile, sest üleval on infokast.
    return gridX * TILE, gridY * TILE + HUD_HEIGHT


def isWall(gridX, gridY, walls):
    # Kontrollib, kas etteantud ruudus on sein.
    # Kui koordinaat on kaardist väljas, loeme seda samuti seinaks.
...
def loadMap():
    # See funktsioon loeb RAW_MAP listi ja teeb sellest mängu objektid.
    # Eraldi listidesse pannakse seinad, punktid, boonuspunktid ja kummitused.
    walls = []
    pellets = []
    powerPellets = []
    ghosts = []
...
def drawMap(walls, pellets, powerPellets):
    # See funktsioon joonistab kogu mängukaardi.
    # Kõigepealt joonistatakse seinad, siis punktid ja lõpuks boonuspunktid.
...
def drawPlayer(playerX, playerY, directionX):
    # See funktsioon joonistab mängija.
    # Mängija on kollane ring, mis meenutab Pac-Mani.
    # directionX abil saab aru, kummale poole suu võiks jääda.
...
def drawGhosts(ghosts, powerMode):
    # See funktsioon joonistab kummitused.
    # Kui powerMode on aktiivne, muutuvad kummitused halliks.
    # See näitab mängijale, et neid saab ajutiselt süüa.
    for ghost in ghosts:
...
def drawHud(powerMode):
    # See funktsioon joonistab ülemise infokasti.
    # Seal näidatakse skoori, elusid, taset ja vajadusel power mode aega.
...
def getPossibleDirections(x, y, walls):
    # Leiame kõik suunad, kuhu kummitus saab liikuda.
    # Kummitus ei tohi minna seina sisse.
    possible = []
...
def moveGhosts(ghosts, walls):
    # See funktsioon liigutab kõiki kummitusi.
    # Kummitused valivad juhuslikult ühe lubatud suuna.
    # See on lihtsam kui päris Pac-Mani tehisintellekt,
    # aga mängu jaoks piisab ja teeb mängu ettearvamatuks.
    for ghost in ghosts:
...
def resetPositions(playerStart, ghosts):
    # Kui mängija kaotab elu, pannakse mängija ja kummitused tagasi algusesse.
...
def gameLoop():
    ...
    while not gameover:
        ...
        # sündmused
        # Siin kontrollitakse klaviatuuri ja akna sulgemist.
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # menüüs alustab ENTER mängu
                if gameState == "menu" and event.key == pygame.K_RETURN:
                    gameState = "play"

                # lõpu ekraanil alustab ENTER uue mängu
                ...
                # võidu ekraanil alustab ENTER uue mängu
                ...
                # P paneb mängu pausile või võtab pausi maha
                ...
                # nooleklahvid määravad järgmise liikumissuuna
                elif gameState == "play":
        ...
        # mängija liikumine
        # Mängija liigub kindla intervalliga, mitte iga kaader.
        # See jätab ruudustiku liikumise selgeks ja kontrollitavaks.
        moveCounter += 1
        ghostCounter += 1
        ...
        # taseme lõpetamine
        # Kui kõik punktid ja boonuspunktid on söödud,
        # liigub mäng edasi järgmisele tasemele.
